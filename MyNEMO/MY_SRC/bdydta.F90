...
!NEMO3.6
!#if defined key_tide
!        IF (lk_dynspg_ts) THEN      ! Fill temporary arrays with slow-varying bdy data
...
!                       IF ( dta_bdy(ib_bdy)%ll_ssh )  &
!                          & dta_bdy_s(ib_bdy)%ssh(1:ilen1(1)) =dta_bdy(ib_bdy)%ssh(1:ilen1(1))
!                       IF ( dta_bdy(ib_bdy)%ll_u2d )  &
!                          & dta_bdy_s(ib_bdy)%u2d(1:ilen1(2)) =dta_bdy(ib_bdy)%u2d(1:ilen1(2))
!                       IF ( dta_bdy(ib_bdy)%ll_v2d )  &
!                          & dta_bdy_s(ib_bdy)%v2d(1:ilen1(3)) =dta_bdy(ib_bdy)%v2d(1:ilen1(3))
!                    ENDIF
!              ENDIF
!           END DO
!        ELSE ! Add tides if not split-explicit free surface else this is done ints loop
!            !
