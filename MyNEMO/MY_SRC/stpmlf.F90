...
# endif
      ...
      DO jtile = 1, nijtile
         ...
         IF(  lk_asminc .AND. ln_asmiau .AND. &
            & ln_sshinc )   CALL ssh_asm_inc( kstp, Nbb, Nnn )
         IF(  lk_asminc .AND. ln_asmiau .AND. &
         & (  ln_seaiceinc .OR. ln_icethicinc)  )   CALL seaice_asm_inc( kstp )! apply ice assimil
...
!!
      ...
      IF( ln_obsmisfits .AND. nstop == 0 ) &
         &               CALL obs_misfits( kstp, Nnn )
