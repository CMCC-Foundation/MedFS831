      ...
      !
          IF( lk_asminc ) THEN
              IF( ln_asmdin ) THEN
                ...
                IF( ln_trainc ) CALL tra_asm_inc( nit000 - 1, Nbb, Nnn, ts, Nrhs )      ! Tracers
                IF( ln_dyninc ) CALL dyn_asm_inc( nit000 - 1, Nbb, Nnn, uu, vv, Nrhs )  ! Dynamics
                IF( ln_sshinc ) CALL ssh_asm_inc( nit000 - 1, Nbb, Nnn )                ! SSH
                IF( lk_asminc .AND. ln_asmiau .AND. &
                & ( ln_seaiceinc .OR. ln_icethicinc) )   CALL seaice_asm_inc(nit000 -1) ! ICE
            ENDIF
          ENDIF
