   ...
   SUBROUTINE fld_def( sdjf, ldprev, ldnext )
      ...
      ELSE                                                                       ! higher frequency mean (in hours)
         ...
         DO jt = 0, sdjf%nreclast
            ...
            WRITE(numout,*)   'jt', jt , ' nrecsec ' , sdjf%nrecsec(jt)
         END DO
      ENDIF
      !
...
#ifdef key_mfs
         IF(TRIM(sdjf%clvar) == 'u10' .OR. TRIM(sdjf%clvar) == 'v10' .OR. TRIM(sdjf%clvar) == 'rh' .OR. TRIM(sdjf%clvar) == 't2' &
           & .OR. TRIM(sdjf%clvar) == 'precip' .OR. TRIM(sdjf%clvar) == 'msl' .OR. TRIM(sdjf%clvar) == 'clc' & 
           & .OR. TRIM(sdjf%clvar) == 'analysed_sst' &
           & .OR. TRIM(sdjf%clvar) == 'sorunoff' .OR. TRIM(sdjf%clvar) == 's_river') THEN
! TEst comment aimie interpolation not for ECMWF
             sdjf%nrecsec(1:sdjf%nreclast) = sdjf%nrecsec(0:sdjf%nreclast-1)
! AIMIE add this line to check the time fix for ECMWF ( instantaneous or average)
!         WRITE(numout,*)   ' nrecsec after interpolation' , sdjf%nrecsec(1:sdjf%nreclast)
!         WRITE(numout,*)   ' clvar ', sdjf%clvar, 'clrootname ', sdjf%clrootname
         ELSE
