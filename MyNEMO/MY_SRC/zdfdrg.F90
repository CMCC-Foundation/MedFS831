...
!!JC: possible WAD implementation should modify line below if layers vanish
      ...
      ELSE                                            !==  standard Cd  ==!
         ...
         END_2D
      ENDIF
!      !Emanuela Clementi increase of bottom friction re-added in tidal eas6 runs
!# ifdef key_mfs
!             ! Artificially increase the bottom friction in the portion of the
!             ! Atlantic box 
!             ! close to Gibraltar (P.Oddo)
!!          IF((kt==nit000).AND.lwp) WRITE(numout,*) ' Artificially increase the bottom friction in the &
!                                 & portion of the Atlantic box close to Gibraltar (MED24) '
!!CDIR NOVERRCHK
!!         DO jj = mj0(76), mj1(166)
!         DO jj = mj0(114+nn_hls), mj1(152+nn_hls)
!!CDIR NOVERRCHK
!!            DO ji = mi0(224), mi1(299)
!            DO ji = mi0(287+nn_hls-1), mi1(299+nn_hls-1)
!               pCdU(ji,jj) = pCdU(ji,jj) * 5.0
!            END DO
!         END DO
!# endif
      !
