MODULE sbccpl
   ...
   USE sbcblk , ONLY : sf , jp_tair !Aimie ast WW3
   ...
   INTEGER, PARAMETER ::   jps_ast = 39   ! air sea temp diff (K) Aimie add ast WW3
! AIMIE CHANGE add coupling air-sea temp diff 
   ...
   !                                   ! Send to waves
   TYPE(FLD_C) ::   sn_snd_ifrac, sn_snd_crtw, sn_snd_wlev, sn_snd_ast !AIMIE send air-sea temp diff (ast for WW3)
   !                                   ! Received from waves
...
CONTAINS
   ...
   SUBROUTINE sbc_cpl_init( k_ice )
      ...
      NAMELIST/namsbc_cpl/  nn_cplmodel  , ln_usecplmask, nn_cats_cpl , ln_scale_ice_flux,             &
         ...
         &                  sn_snd_ifrac , sn_snd_crtw  , sn_snd_wlev , sn_snd_ast,                    & ! AIMIE send ast to WW3
         &                  sn_rcv_hsig  , sn_rcv_phioc ,  &
...
!!       WRITE(numout,*)'      transmitted solar thru sea-ice  = ', TRIM(sn_rcv_qtrice%cldes), ' (', TRIM(sn_rcv_qtrice%clcat), ')'
      ...
      WRITE(numout,*)'      surface waves:'
         ...
         WRITE(numout,*)'                      - mesh          = ', sn_snd_crtw%clvgrd
         WRITE(numout,*)'      water level                     = ',TRIM(sn_snd_wlev%cldes  ), ' (', TRIM(sn_snd_wlev%clcat  ), ')'
         !AIMIE add ast for WW3
         WRITE(numout,*)'      air sea temperature difference  = ',TRIM(sn_snd_ast%cldes   ), ' (', TRIM(sn_snd_ast%clcat   ), ')' 
      ENDIF
...
! Start added by Aimie ast for WW3
      !                                                      -----------------------------------------
      !                                                      !     air sea temperature difference (K)    !
      !                                                      -----------------------------------------
      ssnd(jps_ast)%clname = 'O_AST' ;  IF( TRIM(sn_snd_ast%cldes) == 'coupled')   ssnd(jps_ast)%laction = .TRUE.
! End added by Aimie ast for WW3
...
! Aimie  add ast for WW3
      !                                     !--------------------------------------------
      !                                     !
      !                                     !   AST : air sea temperature difference (in K)  
      !                                     !--------------------------------------------
      IF( ssnd(jps_ast)%laction ) THEN
      ! sf(jp_tair )%fnow(:,:,1) ... Air temperature at 2 m [Kelvin] from input data
      ! ts(:,:,1,jp_tem,Kmm) ... Sea surface temperature  [Celsius]
      ! rto ...= 273.15 parameter to convert [Celsius] to [Kelvin]

               ztmp1(:,:) = ((sf(jp_tair )%fnow(:,:,1)-rt0)- ts(:,:,1,jp_tem,Kmm))* tmask(:,:,1)
            CALL cpl_snd( jps_ast  , isec, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
      ENDIF
! End Aimie
      !
