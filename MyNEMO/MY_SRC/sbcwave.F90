...
!AIMIE add key to have stokes drift calculated but not used as in EAS7
         &                  ln_sdw_tradyn
     !!---------------------------------------------------------------------
      IF(lwp) THEN
         WRITE(numout,*)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_wave in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Stokes drift used in tracer adv and sfc press gdt div ln_sdw_tradyn = ', ln_sdw_tradyn
