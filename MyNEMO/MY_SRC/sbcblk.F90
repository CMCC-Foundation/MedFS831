MODULE sbcblk
   ...
   USE sbcblk_algo_mfs      ! => turb_mfs     : MFS
   !
   ...
   iNTEGER, PARAMETER ::   np_MFS       = 6   ! "MFS" algorithm
   !#LB:
#if defined key_si3
   ! Same, over sea-ice:
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
      ...
      ENDIF
      IF( ln_MFS ) THEN
         slf_i(jp_qsr)%clname = 'NOT USED'
         slf_i(jp_qlw)%clname = 'NOT USED'
      ENDIF
         ...
         IF( TRIM(sf(jfpr)%clrootname) == 'NOT USED' ) THEN    !--  not used field  --!   (only now allocated and set to default)
            ...
            ELSEIF( jfpr ==jp_qsr .OR. jfpr==jp_qlw ) THEN
               IF( ln_MFS ) THEN
                  DEALLOCATE( sf(jfpr)%fnow )   ! deallocate as not used in this case
               ELSE
                  !! AIMIE TEST ECMWF WITHOUT QSR AND QLW
                   sf(jp_qsr)%fnow(:,:,1:ipka) = 80._wp
                   sf(jp_qlw)%fnow(:,:,1:ipka) = 300._wp
               ENDIF
            ELSE
      ...
      IF(lwp) THEN                     !** Control print
         ...
         WRITE(numout,*) '      "MFS"       algorithm                                   ln_MFS   = ', ln_MFS
         ...
         CASE( np_MFS       )   ;   WRITE(numout,*) '   ==>>    "MFS" algorithm     ()'
         END SELECT
         !
         WRITE(numout,*)
   ...
   SUBROUTINE sbc_blk( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
       IF(ln_MFS) THEN
         CALL blk_oce_mfs( kt , sf(jp_wndi )%fnow(:,:,1), sf(jp_wndj)%fnow(:,:,1),& 
            &                sf(jp_tair )%fnow(:,:,1), sf(jp_prec)%fnow(:,:,1),sf(jp_prec)%freqh, &
            &                sf(jp_slp  )%fnow(:,:,1), sf(jp_cc )%fnow(:,:,1), & 
            &                sf(jp_humi )%fnow(:,:,1) , sst_m)
       ELSE
