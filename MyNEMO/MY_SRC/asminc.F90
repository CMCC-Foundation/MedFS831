!ToDo: check why background variables are removed.
MODULE asminc
   !!======================================================================
   !!                       ***  MODULE asminc  ***
   ...
   LOGICAL, PUBLIC :: ln_icethicinc !: No sea ice thickness increment
...
!   REAL(wp), PUBLIC, DIMENSION(:,:,:), ALLOCATABLE ::   u_bkg   , v_bkg      !: Background u- & v- velocity components
   ...
   REAL(wp), DIMENSION(:,:), ALLOCATABLE ::   ssh_bkginc !,ssh_bkg    ! Background sea surface height and its increment
   REAL(wp), DIMENSION(:,:), ALLOCATABLE ::   seaice_bkginc         ! Increment to the background sea ice conc
   REAL(wp), DIMENSION(:,:), ALLOCATABLE ::   icethic_bkginc        ! Increment to the background sea ice thuick
#if defined key_si3
   REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::   fa_i,fa_i_b,fv_i         !
   REAL(wp), DIMENSION(:,:), ALLOCATABLE :: at_i_inc_b,at_i_inc,zofrld
   REAL(wp), DIMENSION(:,:), ALLOCATABLE :: further_corr_finc,ht_i_inc,hm_i_new
#endif
...
CONTAINS
   ...
   SUBROUTINE asm_inc_init( Kbb, Kmm, Krhs )
      ...
      NAMELIST/nam_asminc/ ln_bkgwri,                                      &
         &                 ln_trainc, ln_dyninc, ln_sshinc, ln_icethicinc, &
         &                 ln_seaiceinc,ln_asmdin, ln_asmiau,              &
         &                 nitbkg, nitdin, nitiaustr, nitiaufin, niaufn,   &
         &                 ln_salfix, salfixmin,ln_temnofreeze, nn_divdmp
      !!----------------------------------------------------------------------
      ...
      nitdin    = 0
      nitiaustr = 1
      nitiaufin = 180      ! = 6 hours in 1/24 MedFS configuration
      niaufn    = 0
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Logical switch for ice thickness increments              ln_icethicinc =  ', ln_icethicinc
   ...
   ALLOCATE(fa_i(jpi,jpj,jpl),fa_i_b(jpi,jpj,jpl),fv_i(jpi,jpj,jpl))
   ALLOCATE(at_i_inc_b(jpi,jpj),at_i_inc(jpi,jpj),zofrld(jpi,jpj))
   ALLOCATE(further_corr_finc(jpi,jpj),ht_i_inc(jpi,jpj),hm_i_new(jpi,jpj))
   ENDIF
#endif
...
!
         IF ( ln_trainc ) THEN
            ...
            CALL iom_get( inum, jpdom_auto, 'tem', t_bkginc, 1 )
            CALL iom_get( inum, jpdom_auto, 'sal', s_bkginc, 1 )
            ! Apply the masks
            ...
            !CALL lbc_lnk("asm_inc_init", t_bkginc, 'T', 1. )   ! lateral boundarycond. (no sign change)
            !CALL lbc_lnk("asm_inc_init", s_bkginc, 'T', 1. )   ! lateral boundary cond. (nosign change)
         ENDIF

         IF ( ln_dyninc ) THEN
            ALLOCATE( u_bkginc     (jpi,jpj,jpk) )   ;   u_bkginc     (:,:,:) = 0._wp
            ALLOCATE( v_bkginc     (jpi,jpj,jpk) )   ;   v_bkginc     (:,:,:) = 0._wp
            CALL iom_get( inum, jpdom_auto, 'uvl', u_bkginc, 1 )
            CALL iom_get( inum, jpdom_auto, 'vvl', v_bkginc, 1 )
            ! Apply the masks
         ...
         IF ( ln_sshinc ) THEN
           ...
           CALL iom_get( inum, jpdom_auto, 'eta', ssh_bkginc, 1 )
            ! Apply the masks
            ssh_bkginc(:,:) = ssh_bkginc(:,:) * tmask(:,:,1)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            WHERE( ABS( ssh_bkginc(:,:) ) > 1.0e+10 ) ssh_bkginc(:,:) = 0.0
            !CALL lbc_lnk("asm_inc_init", ssh_bkginc, 'T', 1. )   ! lateral boundary cond. (no sign change)

         ENDIF

         IF ( ln_seaiceinc ) THEN
           ALLOCATE( seaice_bkginc(jpi,jpj)     )   ;   seaice_bkginc(:,:)   = 0._wp
           ALLOCATE( icethic_bkginc(jpi,jpj)    )   ;   icethic_bkginc(:,:) = 0._wp
           CALL iom_get( inum, jpdom_auto, 'INCICECOV', seaice_bkginc, 1 )
           IF ( ln_icethicinc ) CALL iom_get( inum, jpdom_auto, 'INCICETHI', icethic_bkginc, 1 )
            ! Apply the masks
            seaice_bkginc(:,:) = seaice_bkginc(:,:) * tmask(:,:,1)
            icethic_bkginc(:,:) = icethic_bkginc(:,:) * tmask(:,:,1)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            WHERE( ABS( seaice_bkginc(:,:) ) > 1.0e+10 ) seaice_bkginc(:,:) = 0.0
            WHERE( ABS( icethic_bkginc(:,:) ) > 1.0e+10 ) icethic_bkginc(:,:) =0.0
            !CALL lbc_lnk("asm_inc_init", seaice_bkginc, 'T', 1. )   ! lateral boundary cond.(no sign change)
            !CALL lbc_lnk("asm_inc_init", icethic_bkginc, 'T', 1. )   ! lateral boundary cond.(no sign change)
         ENDIF
         !
         CALL iom_close( inum )
         !
   ...
   SUBROUTINE tra_asm_inc( kt, Kbb, Kmm, pts, Krhs )
      ...
      IF ( ln_asmiau ) THEN            ! Incremental Analysis Updating
         ...
         IF ( ( kt >= nitiaustr_r ).AND.( kt <= nitiaufin_r ) ) THEN
            ...
            END DO
            !
            !CALL lbc_lnk("tra_asm_inc", pts(:,:,:,:,Krhs), 'T', 1. )   ! lateral boundarycond. (no sign change)
      ...
      ELSEIF ( ln_asmdin ) THEN        ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            IF(lwp) THEN
               WRITE(numout,*)
               WRITE(numout,*) 'tra_asm_inc : Applying Analysis Increments',&
               & ' at time step = ', kt
               WRITE(numout,*) '~~~~~~~~~~~~'
            ENDIF
            ...
            IF (ln_temnofreeze) THEN
               ...
               WHERE( t_bkginc(:,:,:) > 0.0_wp .OR. pts(:,:,:,jp_tem,Kmm) + t_bkginc(:,:,:) > fzptnz(:,:,:) )
                  pts(:,:,:,jp_tem,Kmm) = pts(:,:,:,jp_tem,Kmm) + t_bkginc(:,:,:)
               END WHERE
            ELSE
               pts(:,:,:,jp_tem,Kmm) = pts(:,:,:,jp_tem,Kmm) + t_bkginc(:,:,:)
            ENDIF
            IF (ln_salfix) THEN
               ...
               WHERE( s_bkginc(:,:,:) > 0.0_wp .OR. pts(:,:,:,jp_sal,Kmm) + s_bkginc(:,:,:) > salfixmin )
                  pts(:,:,:,jp_sal,Kmm) = pts(:,:,:,jp_sal,Kmm) + s_bkginc(:,:,:)
               END WHERE
            ELSE
               pts(:,:,:,jp_sal,Kmm) = pts(:,:,:,jp_sal,Kmm) + s_bkginc(:,:,:)
            ENDIF
   ...
   SUBROUTINE dyn_asm_inc( kt, Kbb, Kmm, puu, pvv, Krhs )
      ...
      ELSEIF ( ln_asmdin ) THEN     ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            !
            ! Initialize the now fields with the background + increment
            puu(:,:,:,Kmm) = puu(:,:,:,Kmm) + u_bkginc(:,:,:)
            pvv(:,:,:,Kmm) = pvv(:,:,:,Kmm) + v_bkginc(:,:,:)
            !
   ...
   SUBROUTINE ssh_asm_inc( kt, Kbb, Kmm )
      ...
      ELSEIF ( ln_asmdin ) THEN     ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            l_1st_euler = .TRUE.                            ! Force Euler forward step
            !
            ssh(:,:,Kmm) = ssh(:,:,Kmm) + ssh_bkginc(:,:)   ! Initialize the now fields the background + increment
            !
...
!!gm why not e3u(:,:,:,Kbb), e3v(:,:,:,Kbb), gdept(:,:,:,Kbb) ????
   ...
   SUBROUTINE seaice_asm_inc( kt, kindic )
      ...
      INTEGER  ::   it
      REAL(wp) ::   zincwgt   ! IAU weight for current time step
      REAL(wp) ::   zhicifmin = 0.1_wp      ! ice minimum depth in metres
      !!----------------------------------------------------------------------
      !
      !                             !-----------------------------------------
      IF ( ln_asmiau ) THEN         ! Incremental Analysis Updating
         ...
         IF ( ( kt >= nitiaustr_r ).AND.( kt <= nitiaufin_r ) ) THEN
            ...
            DO jl = 1, jpl
                !initial definition of fraction vol, sic and sic_b that is the repvios value
               WHERE(at_i(:,:)> epsi20) ;  fa_i(:,:,jl)= a_i(:,:,jl)/at_i(:,:)
               ELSEWHERE
                        fa_i(:,:,jl)=0._wp
                        fa_i(:,:,1)=1._wp
               ENDWHERE
               WHERE(at_i_b(:,:)> epsi20) ;  fa_i_b(:,:,jl)= a_i_b(:,:,jl)/at_i_b(:,:)
               ELSEWHERE
                    fa_i_b(:,:,jl)=0._wp
                    fa_i_b(:,:,1)=1._wp
               ENDWHERE

               WHERE(vt_i(:,:)> epsi20) ; fv_i(:,:,jl)=v_i(:,:,jl)/vt_i(:,:)
               ELSEWHERE
                        fv_i(:,:,jl)=0._wp
                        fv_i(:,:,1)=1._wp
               ENDWHERE
            ...
            ! rn_amax_2d max concentrtion (can differ for SH and NH, read by namelist)
            ! defition of increments (not divided by cathegory)
            DO_2D( 0, 0, 0, 0 )
                !before timestep only sic

                zofrld (ji,jj) = rn_amax_2d(ji,jj) - at_i_b(ji,jj)    !fraction of ocean rather that fraction of sea ice
                fr_i (ji,jj) = 1._wp - MIN( MAX( zofrld(ji,jj) - seaice_bkginc(ji,jj) * zincwgt, 0.0_wp), 1._wp)
                at_i_inc_b(ji,jj)=fr_i(ji,jj)-at_i(ji,jj)
                at_i_b(ji,jj)=fr_i (ji,jj)
                !current timestep
                zofrld (ji,jj) = rn_amax_2d(ji,jj) - at_i(ji,jj)    !fraction of ocean rather that fraction of sea ice
                fr_i  (ji,jj) = 1._wp - MIN( MAX( zofrld(ji,jj) - seaice_bkginc(ji,jj) * zincwgt, 0.0_wp), 1._wp)
                at_i_inc(ji,jj)=fr_i(ji,jj)-at_i(ji,jj)
                at_i(ji,jj)=fr_i(ji,jj)

                hm_i_new(ji,jj) = MAX( hm_i(ji,jj) + icethic_bkginc(ji,jj) * zincwgt,0.0_wp)

                IF((at_i_inc(ji,jj) > 0.0_wp .AND. hm_i_new(ji,jj) <zhicifmin) .OR. (zofrld(ji,jj) > 0.999999_wp .AND. hm_i_new(ji,jj) >zhicifmin ) ) THEN
                    further_corr_finc(ji,jj) = (zhicifmin - hm_i_new(ji,jj)) * zincwgt
                ELSE
                    further_corr_finc(ji,jj) = 0.0_wp
                ENDIF

                hm_i_new (ji,jj) = hm_i_new (ji,jj) + further_corr_finc(ji,jj)
                ht_i_inc(ji,jj)=hm_i_new(ji,jj)-hm_i(ji,jj)
                hm_i(ji,jj)=hm_i_new(ji,jj)

           END_2D
            ! distribution across cathegory

            DO jl = 1, jpl
                !initial definition of fraction vol, sic and sic_b that is the repvios value
                DO_2D( 0, 0, 0, 0 )
                    a_i(ji,jj,jl)=a_i(ji,jj,jl)+fa_i(ji,jj,jl)*at_i_inc(ji,jj)
                    h_i(ji,jj,jl)=h_i(ji,jj,jl)+fv_i(ji,jj,jl)*ht_i_inc(ji,jj)
            ...
            IF( .NOT. l_istiled .OR. ntile == nijtile )  THEN                !Do only on the last tile
                IF ( kt == nitiaufin_r ) THEN
                    DEALLOCATE(fa_i,fa_i_b,fv_i)
                    DEALLOCATE(at_i_inc_b,at_i_inc,zofrld)
                    DEALLOCATE(further_corr_finc,ht_i_inc,hm_i_new)
                ENDIF
            ENDIF
      ...
      ELSEIF ( ln_asmdin ) THEN     ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            DO_2D( 0, 0, 0, 0 )
                ...
                zofrld (ji,jj) = rn_amax_2d(ji,jj) - at_i_b(ji,jj)    !fraction of ocean rather that fraction of sea ice
                fr_i (ji,jj) = 1._wp - MIN( MAX( zofrld(ji,jj) - seaice_bkginc(ji,jj), 0.0_wp), 1._wp)
                at_i_inc_b(ji,jj)=fr_i(ji,jj)-at_i(ji,jj)
                at_i_b(ji,jj)=fr_i (ji,jj)
                !current timestep
                zofrld (ji,jj) = rn_amax_2d(ji,jj) - at_i(ji,jj)    !fraction of ocean rather that fraction of sea ice
                ...
                hm_i_new(ji,jj) = MAX( hm_i(ji,jj) + icethic_bkginc(ji,jj) ,0.0_wp)

                IF((at_i_inc(ji,jj) > 0.0_wp .AND. hm_i_new(ji,jj) <zhicifmin) .OR. (zofrld(ji,jj) > 0.999999_wp .AND. hm_i_new(ji,jj) >zhicifmin ) ) THEN
                    further_corr_finc(ji,jj) = (zhicifmin - hm_i_new(ji,jj))
                ELSE
                    further_corr_finc(ji,jj) = 0.0_wp
                ENDIF
